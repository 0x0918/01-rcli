...
pub trait TextVerify {
    fn verify(&self, read: impl Read, sig: &[u8]) -> Result<bool>;
}
...
impl TextSign for Blake3 {
    fn sign(&self, reader: &mut dyn Read) -> Result<Vec<u8>> {
        let mut buf = Vec::new();
        reader.read_to_end(&mut buf)?;
        Ok(keyed_hash(&self.key, &buf).as_bytes().to_vec())
    }
}
...
impl TextVerify for Blake3 {
    fn verify(&self, mut reader: impl Read, sig: &[u8]) -> Result<bool> {
        let mut buf = Vec::new();
        reader.read_to_end(&mut buf)?;
        let hash = keyed_hash(&self.key, &buf).as_bytes().to_vec();
        Ok(hash == sig)
    }
}
...
impl KyeGenerator for Ed25519Signer {
    fn generate() -> Result<Vec<Vec<u8>>> {
        let mut rng = OsRng;
...
mod test {
    ...
    use super::{Ed25519Signer, Ed25519Verifier, KeyLoader, TextSign};
